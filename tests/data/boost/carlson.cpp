...
typedef policies::policy<policies::promote_double<false>> f64_policy;
...
int main()
{
    process_carlson(
        ...
        { return ellint_rf(v[0], v[1], v[2], f64_policy()); });
    process_carlson(
        "../wolfram/elliprg_data.csv", "elliprg_data.csv", 3,
        [](const std::vector<double> &v)
        { return ellint_rg(v[0], v[1], v[2], f64_policy()); });
    process_carlson(
        "../wolfram/elliprj_data.csv", "elliprj_data.csv", 4,
        [](const std::vector<double> &v)
        { return ellint_rj(v[0], v[1], v[2], v[3], f64_policy()); });
    process_carlson(
        "../wolfram/elliprj_pv.csv", "elliprj_pv.csv", 4,
        [](const std::vector<double> &v)
        { return ellint_rj(v[0], v[1], v[2], v[3], f64_policy()); });
    process_carlson(
        "../wolfram/elliprj_pv_small.csv", "elliprj_pv_small.csv", 4,
        [](const std::vector<double> &v)
        { return ellint_rj(v[0], v[1], v[2], v[3], f64_policy()); });
    return 0;
}
