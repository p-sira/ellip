...
macro_rules! get_trace {
    ...
    ($phi: expr, $m: expr, $name: expr, $line_color: expr) => {
        get_trace!($phi, $m, $name).line(Line::new().width(2.0).color($line_color))
    };
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    plot.set_layout(
        Layout::new()
            .x_axis(
                Axis::new()
                    .title(Title::with_text("φ").font(Font::new().size(18)))
                    .show_line(true)
            ...
            .y_axis(
                Axis::new()
                    .title(Title::with_text("Z(φ,m)").font(Font::new().size(18)))
                    .show_line(true)
                    .range(vec![-1.0, 1.0]),
            )
            .legend(
    ...
    make_html!(plot, "jacobi_zeta_plot_paper.html");
    write_svg!(plot, "jacobi_zeta_plot_paper.svg", 1000, 600, 1.0);
    println!("Done");
    Ok(())
}
