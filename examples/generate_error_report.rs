 ...
 */
 ...
 <<<<<<<< HEAD:examples/generate_error_report.rs
         let file_path = ["./tests/data/", $file_name, ".csv"].concat();
 ========
         let file_path = ["tests/data/", $file_name, ".csv"].concat();
 >>>>>>>> 93958cd (CHG: Fix coverage workflow, move the generate-error-report back to examples):examples/generate-error-report.rs
