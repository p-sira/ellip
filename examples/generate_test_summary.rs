...
fn main() {
    ...
    // Build summary entries across groups (must match benches group names)
    let legendre = [
    ...
    // Construct final summary markdown
    ...
    summary_sections.push("### Legendre's Elliptic Integrals".to_string());
    summary_sections.push(generate_summary_table(&legendre));

    summary_sections.push("".to_string());
    summary_sections.push("### Carlson's Symmetric Integrals".to_string());
    summary_sections.push(generate_summary_table(&carlson));

    summary_sections.push("".to_string());
    summary_sections.push("### Bulirsch's Elliptic Integrals".to_string());
    summary_sections.push(generate_summary_table(&bulirsch));

    let summary_markdown = summary_sections.join("\n");

    // Replace placeholder
    let output = template.replace("{{TESTSUMMARY}}", &summary_markdown);
